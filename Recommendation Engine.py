...
# Replacing '|' in the 'rating' column with the most frequent rating
...
# Building the KNNBasic algorithm with Item-Based Collaborative Filtering
...
def hybrid_recommendations(user_id, product_id):
    # Getting collaborative filtering recommendations using KNNBasic (Item-Based)
    collab_rec = [pred.iid for pred in knn_algo.test([[user_id, product_id, 4]])]

    # Getting content-based filtering recommendations
    content_rec = content_based_recommendations(product_id)

    # Combining the results and remove duplicates
    recommendations = collab_rec + content_rec
    recommendations = list(set(recommendations))

    # Removing the input product_id from recommendations if it exists
